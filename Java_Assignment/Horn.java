...
public class Horn extends Solid{

    public Horn () { System.out.println("뿔 도형을 선택하셨습니다.\n"); }

    BufferedReader scan = new BufferedReader(new InputStreamReader(System.in));
    BufferedWriter print = new BufferedWriter(new OutputStreamWriter(System.out));
    String is;

    public boolean setting () throws IOException { //뿔의 밑면넓이와 높이 설정

        while (true) {
            try {
                print.write("구하시려는 뿔에서 밑면의 넓이를 입력해주세요.(숫자만 입력하셔야 합니다.)\n");
        ...
        while (true) {
            try {
                print.write("뿔의 높이를 입력해주세요.\n");
                print.flush();
                setfHeight(Double.parseDouble(scan.readLine()));
                if (getfHeight() < 0) {
                    print.write("뿔의 넓이는 음수의 값을 가질수 없습니다.\n다시 입력하셔야 합니다.\n");
                }
                break;
            } catch (Exception e) {
        ...
        setfBase_Volume(0.33333);
        return true;

    }

    public void getting () throws IOException { //뿔의 부피계산 및 출력(소수점 5자리 고정)
        double volume = getfBase_Volume() * getfFoot_Width() * getfHeight();
        print.write("뿔의 부피는 " + String.format("%.5f",volume) + is);
        print.flush();
    }
